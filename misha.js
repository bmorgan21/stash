...
LocationSchema.virtual("coords").get(function() {
  return {
    ...
    this.location[0] = map.lon;
    return this.location[1] = map.lat;
  });
...
lazy(fs.createReadStream(path.join(__dirname, 'datafiles', 'cities.csv')))
  ...
  .map(function (line) {
    ...
    location.save((function (err) {
      ...
      count++;
      var state = location.country == 'US' ? ', ' + location.state + ' ' : ', ';
      console.log(msg + ((count / 353224) * 100).toFixed(2) + ' %  | ' + location.normalizedCity + " (" + location.country + ") ")
    }));
  });
